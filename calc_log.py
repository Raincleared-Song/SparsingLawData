...
def main(model: str):
    ...
    if '1.2b' in model:
        popt, _ = curve_fit(test_func, x[6:], y[6:], maxfev=40000)
    else:
        popt, _ = curve_fit(test_func, x, y, maxfev=40000)
    ...
    ax.scatter(x, y)
    TEST_FORMAT = r'${c:.2f} \cdot \log({a:.2f} D + {b:.2f}) + {d:.3f}$'
